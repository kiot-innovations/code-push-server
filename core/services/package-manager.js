...
proto.createDeploymentsVersionIfNotExist = function (
    deploymentId,
    appVersion,
    minVersion,
    maxVersion,
            ...
            .then(() => {
                ...
                if (isUseDiffText == constConfig.IS_USE_DIFF_TEXT_YES) {
                    ...
                    _.forEach(json.diff, function (tmpFilePath) {
                        var dataCenterContentPathTmpFilePath = path.join(
                            dataCenterContentPath,
                            tmpFilePath,
                        );
                        var oldPackageDataCenterContentPathTmpFilePath = path.join(
                            oldPackageDataCenterContentPath,
                            tmpFilePath,
                        );
                        if (
                            ...
                            var textOld = fs.readFileSync(
                                oldPackageDataCenterContentPathTmpFilePath,
                                'utf-8',
                            );
                            var textNew = fs.readFileSync(
                                dataCenterContentPathTmpFilePath,
                                'utf-8',
                            );
                            if (!textOld || !textNew) {
                                return;
            ...
            .then(([currentPackageInfo, rollbackPackageInfos]) => {
                if (currentPackageInfo && rollbackPackageInfos.length > 0) {
                    for (var i = rollbackPackageInfos.length - 1; i >= 0; i--) {
                        if (
                            rollbackPackageInfos[i].package_hash != currentPackageInfo.package_hash
                        ) {
